...
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-xs-16 text-center pt-3">
            ...
            {{ Form::button('Bloqueios', ['type'  => 'submit', 'name'  => 'genero', 'value' => 'bloqueio', 'class' => 'btn btn-warning active']) }}
            ...
            <table class="table table-condensed valign-middle">
                <thead>
                    <tr>
                        ...
                        <th style="width:80px;" title="Hora Inicial">H. Inicial</th>
                        <th style="width:80px;" title="Hora Final">H. Final</th>
                        <th>Descrição Bloqueio</th>
                        <th style="width:150px;">Profissional</th>
                        <th style="width:150px;" title="Usuário logado que cadastrou o bloqueio">Usuário</th>
                        <th style="width:150px;" title="Indicação manual de quem cadastrou o bloqueio">Cadastrado por</th>
                ...
                <tbody>
                    @foreach ($agenda as $row)
                        <tr>
                            ...
                            <td>{{ $row->inicio }}</td>
                            <td>{{ $row->fim }}</td>
                            <td>{{ $row->descricao_bloqueio }}</td>
                            <td>
                                @if($row->terapeutabloqueio)
                                    {{ $row->terapeutabloqueio->name }}
                                @endif
                            </td>
                            <td>
                                @if($row->createdBy)
                            ...
                            <td>
                                <a class="confirm-destroy" href="{{ route('agendasDestroyBloqueio', array('id' => $row->id)) }}">
